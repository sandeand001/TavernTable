...
import {
  ...
  globalStriations as motifGlobalStriations,
  globalCracks as motifGlobalCracks,
  scatterBlobsGlobal as motifScatterBlobsGlobal,
...
export class BiomeCanvasPainter {
  ...
  _valueNoise2D(nx, ny) {
    ...
    const ix = Math.floor(nx + seedX),
      iy = Math.floor(ny + seedY);
    const fx = nx - ix,
      fy = ny - iy;
    const s = (ix, iy) => {
  ...
  paint(biomeKey, heights, tilesHiddenCallback = null) {
    ...
    const h = this.gameManager.tileHeight;
    const perf =
      typeof window !== 'undefined' && window.richShadingSettings?.performance ? 0.6 : 1.0;
    const densityMul =
      (typeof window !== 'undefined' && window.richShadingSettings?.density) || 1.0;
    const intensity =
      (typeof window !== 'undefined' && window.richShadingSettings?.intensity) || 1.0;
    const mapFreq = (typeof window !== 'undefined' && window.richShadingSettings?.mapFreq) || 0.05;
    const shorelineSandStrength =
      typeof window !== 'undefined' &&
      Number.isFinite(window.richShadingSettings?.shorelineSandStrength)
        ? Math.max(0, window.richShadingSettings.shorelineSandStrength)
        : 1.0;
    ...
    this._slopeGainForStroke =
      typeof window !== 'undefined' && Number.isFinite(window.richShadingSettings?.slopeGain)
        ? Math.max(0.1, window.richShadingSettings.slopeGain)
        : 1.5;
    ...
    for (let d = minDepth; d <= maxDepth; d++) {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          const color = getBiomeColorHex(biomeKey, heightVal, x, y, {
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          const color = getBiomeColorHex(biomeKey, hv, x, y, {
            moisture: moisture?.[y]?.[x] ?? 0.5,
            slope: slope?.[y]?.[x] ?? 0.0,
            aspectRad: aspect?.[y]?.[x] ?? 0.0,
            seed,
          ...
          const a = 0.2 * perf; // keep alpha stable; intensity affects color contrast in palette
          this._strokeBlob(ctx, px, py, r, color, a, 0.1 + 0.1 * rJitter, 28);
          const a2 = 0.14 * perf;
    ...
    for (const [d, layer] of this.layers.entries()) {
      if (!paintedDepths.has(d)) {
        if (layer.sprite && !layer.sprite.destroyed) {
          try {
            this.gameManager.gridContainer.removeChild(layer.sprite);
          } catch {
