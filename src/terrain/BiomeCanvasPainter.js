...
export class BiomeCanvasPainter {
    constructor(gameManager) {
        ...
        this._seed = Math.floor(Math.random() * 1e9) >>> 0;
    }
    _hex(colorInt) {
    ...
    _bandOrientationForDepth(d, heights, slope, aspect, predicateFn = null) {
        const cols = this.gameManager.cols;
        const rows = this.gameManager.rows;
    ...
    ctx.strokeStyle = this._hex(color);
        ...
        for (let d = 0; d < len; d += step) {
            const angle = orient + (this._valueNoise2D(x * 0.01, y * 0.01) * 0.7);
    ...
    _drawIceGlintsInFace(ctx, cx, cy, w, h, alpha = 0.18) {
        ...
        const count = 2 + Math.floor(this._valueNoise2D(cx * 0.03, cy * 0.03) * 2 + 2);
        for (let i = 0; i < count; i++) {
            const ox = (Math.random() * 0.3 + 0.1) * w * 0.5;
            const oy = (Math.random() * 0.2 - 0.1) * h * 0.5;
            const x0 = cx + ox;
            const y0 = cy - oy;
            const x1 = x0 + w * 0.18;
            const y1 = y0 - h * 0.14;
    ...
    ctx.strokeStyle = this._hex(color);
        ...
        for (let i = 0; i < density; i++) {
            const ang = -Math.PI / 3 + (Math.random() - 0.5) * 0.3;
            const r = (Math.random() * 0.25 + 0.15) * Math.min(w, h) * 0.5;
    ...
    _drawCanopyInFace(ctx, cx, cy, w, h, baseColor, alpha = 0.18) {
        ...
        const count = 3 + Math.floor(this._valueNoise2D(cx * 0.02, cy * 0.02) * 2 + 2);
        for (let i = 0; i < count; i++) {
            const ox = (Math.random() - 0.5) * w * 0.2;
            const oy = (Math.random() - 0.1) * h * 0.25;
            this._strokeBlob(ctx, cx + ox, cy + oy, r * (0.8 + Math.random() * 0.6), crownColor, alpha * (0.9 + Math.random() * 0.2), 0.1, 18);
        }
    ...
    _globalRibbons(ctx, canvas, count, width, color, alpha, orient = 0) {
        for (let i = 0; i < count; i++) {
            const y0 = (i + 0.5) / (count + 1) * canvas.height;
            const x0 = (i % 2 === 0) ? 0 : canvas.width * 0.12;
    ...
    ctx.strokeStyle = this._hex(this._shadeHex(color, 0.4));
        ...
        for (let i = 0; i < count; i++) {
            ...
            for (let t = 0; t < len; t += step) {
                x += Math.cos(theta + (Math.random() - 0.5) * jitter) * step;
                y += Math.sin(theta + (Math.random() - 0.5) * jitter) * step;
                ctx.lineTo(x, y);
            }
            ctx.stroke();
        }
    ...
    _scatterBlobsGlobal(ctx, canvas, count, rBase, rVar, color, alpha = 0.16, jag = 0.08, steps = 18) {
        for (let i = 0; i < count; i++) {
            ...
            const r = rBase * (1 + Math.random() * rVar);
    ...
    _styleForBiome(biome) {
        ...
        if (/ocean|coast|river|lake|reef|oasis|geyser/i.test(b)) return 'water';
    ...
    paint(biomeKey, heights, tilesHiddenCallback = null) {
        ...
        const intensity = (typeof window !== 'undefined' && window.richShadingSettings?.intensity) || 0.8;
        ...
        for (let d = minDepth; d <= maxDepth; d++) {
            ...
            const bandOrient = this._bandOrientationForDepth(d, heights, slope, aspect);
            ...
            if (style === 'plains') {
                ...
                this._scatterTuftsGlobal(ctx, canvas, Math.floor(density), Math.min(w, h) * 0.35, this._shadeHex(col, 0.75), 0.18 * intensity, 0.12);
            } else if (style === 'arid') {
                const col = getBiomeColorHex(
                    biomeKey,
                    2,
                ...
                if (/salt|flat/i.test(String(biomeKey))) {
                    this._globalCracks(ctx, canvas, col, 0.22 * intensity, 8 + Math.floor(4 * densityMul));
                } else {
                    this._globalRibbons(ctx, canvas, longCount, Math.max(2, h * 0.18), col, 0.10 * intensity, bandOrient + Math.PI);
                    // Some savanna/steppe tufts layered sparsely
                    if (/savanna|steppe|prairie|grass/i.test(String(biomeKey))) {
                        this._scatterTuftsGlobal(ctx, canvas, Math.floor((rows + cols) * 0.4 * perf), Math.min(w, h) * 0.3, this._shadeHex(col, 0.7), 0.16 * intensity, 0.15);
                    }
                }
                ...
                this._scatterBlobsGlobal(ctx, canvas, Math.floor(count), Math.min(w, h) * 0.26, 0.7, this._shadeHex(col, 0.85), 0.16 * intensity, 0.08, 18);
                ...
                const wetOrient = this._bandOrientationForDepth(d, heights, slope, aspect, (x, y) => (heights[y][x] || 0) < 0);
                ...
                this._globalRibbons(ctx, canvas, longCount + 4, Math.max(2, h * 0.16), waterCol, 0.10 * intensity, wetOrient + Math.PI);
                ctx.restore();
                ...
                this._scatterTuftsGlobal(ctx, canvas, Math.floor(reedCount), Math.min(w, h) * 0.32, this._shadeHex(reedCol, 0.6), 0.18 * intensity, 0.08);
                ctx.restore();
                ...
                this._globalStriations(ctx, canvas, midCol, 0.14 * intensity, bandOrient + Math.PI / 2, Math.max(28, Math.floor(h * 0.7)));
                // High/frozen sub-clip for glints
                ctx.save();
                this._applyFaceClip(ctx, this.bounds, heights, (gx, gy) => (gx + gy) === d && (heights[gy][gx] || 0) > 1);
                this._globalStriations(ctx, canvas, 0xffffff, 0.10 * intensity, bandOrient - Math.PI / 3, Math.max(36, Math.floor(h * 0.8)));
                ctx.restore();
            } else if (style === 'water') {
                const col = getBiomeColorHex(
                    biomeKey,
                ...
                this._globalRibbons(ctx, canvas, longCount + 6, Math.max(2, h * 0.16), col, 0.10 * intensity, bandOrient + Math.PI);
                ...
                this._globalCracks(ctx, canvas, col, 0.24 * intensity, 10 + Math.floor(4 * densityMul));
                ...
                this._globalRibbons(ctx, canvas, Math.floor(longCount * 0.7), Math.max(2, h * 0.14), col, 0.10 * intensity, 0.2);
            } else {
                ...
                this._scatterBlobsGlobal(ctx, canvas, Math.floor((rows + cols) * 0.3 * perf), Math.min(w, h) * 0.20, 0.5, col, 0.10 * intensity, 0.05, 14);
            }
            ...
            if (!layer.sprite || layer.sprite.destroyed) {
                ...
                // Place on the top-face plane for this depth; slightly below tile outlines/tokens
                sprite.zIndex = d * 100 + 0; // tiles are depth*100; tokens start at +1
        ...
        for (const [d, layer] of this.layers.entries()) {
            if (!paintedDepths.has(d)) {
                if (layer.sprite && !layer.sprite.destroyed) {
                    try { this.gameManager.gridContainer.removeChild(layer.sprite); } catch { }
                    try { layer.sprite.destroy?.(); } catch { }
                }
