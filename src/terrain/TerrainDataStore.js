...
export class TerrainDataStore {
  ...
  /**
   * Reset both base and working height fields to the provided value (defaults to terrain baseline).
   * Rebuilds the arrays to avoid lingering references or inconsistent lengths.
   */
  resetAll(height = TERRAIN_CONFIG.DEFAULT_HEIGHT) {
    ...
    if (!rows || !cols) {
      // Fallback to existing dimensions if metadata is missing.
