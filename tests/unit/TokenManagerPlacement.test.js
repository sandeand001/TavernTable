...
describe('placeNewToken', () => {
    test('creates creature, positions with elevation, sets zIndex, adds to container, and tracks in collection', () => {
        const gridX = 3, gridY = 2;
        const tileWidth = 64, tileHeight = 32;
        const height = 1;

        const addedChildren = [];
        const gridContainer = {
            addChild: jest.fn(child => { addedChildren.push(child); }),
        ...
        expect(sprite.zIndex).toBe((gridX + gridY) * 100 + 1);
    ...
    test('returns early when creature creation fails', () => {
        const c = {
            ...
            gameManager: { tileWidth: 64, tileHeight: 32, terrainCoordinator: { dataStore: { get: jest.fn(() => 0) } } },
            addTokenToCollection: jest.fn(),
