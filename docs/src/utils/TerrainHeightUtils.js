...
export class TerrainHeightUtils {
  ...
  static getElevationUnit() {
    return Number.isFinite(this._elevationUnitOverride)
      ? this._elevationUnitOverride
      : TERRAIN_CONFIG.ELEVATION_SHADOW_OFFSET;
  }
  /**
  ...
  static createHeightArray(rows, cols, defaultHeight = TERRAIN_CONFIG.DEFAULT_HEIGHT) {
    try {
      ...
      if (!Number.isFinite(defaultHeight)) {
        throw new Error(
          `Invalid defaultHeight parameter: ${defaultHeight}. Must be a finite number.`
        );
      }
      ...
      if (!this.isValidHeight(defaultHeight)) {
        logger.log(
          LOG_LEVEL.WARN,
          'Default height outside valid range, clamping',
          LOG_CATEGORY.SYSTEM,
          {
      ...
      // Create the 2D array
      const heightArray = Array(rows)
        .fill(null)
        .map(() => Array(cols).fill(defaultHeight));
  ...
  static getHeightDifference(heightArray, x1, y1, x2, y2) {
    try {
      ...
      if (
        !this.isValidCoordinate(x1, y1, cols, rows) ||
  ...
  static isValidCoordinate(x, y, maxX, maxY) {
    return Number.isInteger(x) && Number.isInteger(y) && x >= 0 && x < maxX && y >= 0 && y < maxY;
  }
  ...
  static resetArea(
    ...
    endX,
    endY,
    resetHeight = TERRAIN_CONFIG.DEFAULT_HEIGHT
  ) {
    try {
      if (!this.isValidHeightArray(heightArray)) {
        return 0;
