...
function scheduleRollAnimation(manager, mesh, metrics, options = {}) {
    ...
    mesh.position.set(start.x, restHeight + groundLift + arcHeight, start.z);
    ...
    const ensureGroundContact = () => {
        try {
            adjustDieHeightForGround(mesh, threeNamespace, restHeight);
        } catch (_) {
            /* ignore */
        }
    };
    ...
    const step = (ts) => {
        ...
        if (settleTs == null) {
            ...
            mesh.position.y = restHeight + groundLift + arcHeight + Math.max(bounce, 0);
            ...
            if (travelT >= 1) {
                settleTs = ts;
                mesh.position.set(target.x, restHeight + groundLift, target.z);
                ensureGroundContact();
            }
            return;
        }
        ...
        if (wobbleT >= 1) {
            if (!hasSnappedFlat) {
                if (!snapState) {
                    const solution = computeSnapSolution(mesh, threeNamespace, restHeight);
                    if (solution) {
                        snapState = {
                ...
                if (snapState?.targetQuat) {
                    ...
                    ensureGroundContact();
                    if (snapT >= 1) {
                        snapDieToBoard(mesh, threeNamespace, restHeight);
                        snapState = null;
                        hasSnappedFlat = true;
                        stopAnimation();
                    }
...
export async function playD20RollOnGrid(options = {}) {
    ...
    try {
        ...
        const animationState = scheduleRollAnimation(manager, clone, metrics, options);
        activeDieState = {
            mesh: clone,
